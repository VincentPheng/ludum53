...
func _ready():
	...
	numMailboxes = get_tree().get_nodes_in_group("Mailbox").size()
...
func _on_exit_level_passed():
	var score = (numMailboxes * 100) + (lightPkgHealth * 500) + (mediumPkgHealth * 300) + (heavyPkgHealth * 100)
	show_end_level_screen.emit(score)
